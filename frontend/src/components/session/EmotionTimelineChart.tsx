...
const EmotionTimelineChart = ({
  ...
  const processedData = timeline.map((entry, index) => {
    ...
    return {
      value: [new Date(entry.timestamp).getTime(), yPosition],
  ...
  const emotionIds = Object.keys(EMOTION_DATA).map(Number);

  const yAxisMin = Math.min(...emotionIds) - 0.5;
  const yAxisMax = Math.max(...emotionIds) + 0.5;
  const option = {
    backgroundColor: {
      type: 'linear',
      x: 0,
    ...
    tooltip: {
      ...
      extraCssText:
        'backdrop-filter: blur(10px); border-radius: 8px; box-shadow: 0 10px 25px rgba(0,0,0,0.1);',
    ...
    graphic: [
      {
        ...
        children: Array.from({ length: 15 }, (_, i) => ({
          ...
          position: [Math.random() * 600, Math.random() * 300],
          animation: {
    ...
    <div className="relative">
      <div className="mb-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-100">
        ...
        <p className="text-sm text-gray-600">
          Evolución de emociones durante la sesión
        </p>
      </div>
