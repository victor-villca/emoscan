      ...
      <div className="space-y-3">
        {topEmotions.slice(0, 3).map((emotion) => (
          <div key={emotion.name} className="flex items-center">
            <i
              className={`${emotion.icon} mr-3 text-lg text-${emotion.intensity.color}-500`}
            ></i>
            <span className="text-sm text-gray-700 flex-grow font-medium">
              {emotion.name}
            </span>
            <div
              className={`flex items-center text-xs font-bold text-${emotion.intensity.color}-600`}
            >
