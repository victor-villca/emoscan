...
const AnalysisSummary = ({ participantName, emotions, sessionDuration }: AnalysisSummaryProps) => {
  const summaryText = useMemo(() => {
    if (!emotions || emotions.length === 0) {
      return `No emotional data was processed for ${participantName} during this session.`;
    }

    const sortedEmotions = [...emotions].sort((a, b) => parseFloat(b.average_percentage) - parseFloat(a.average_percentage));
  ...
  return (
    <div className="mt-6 bg-gray-50 rounded-lg p-4">
      ...
      <p className="text-gray-700 text-sm" dangerouslySetInnerHTML={{ __html: summaryText }} />
    </div>
  );
};
