...
const EmotionDurationChart = ({ timeline, sessionStartTime, sessionEndTime }: EmotionDurationChartProps) => {
  ...
  const option = {
    ...
    xAxis: { type: 'time', min: new Date(sessionStartTime).getTime(), max: new Date(sessionEndTime).getTime() },
    yAxis: { show: false },
    series: [{
      type: 'custom',
      renderItem: (params: any, api: any) => {
        ...
        return {
          type: 'rect',
          shape: { x: start[0], y: start[1] - height / 2, width: end[0] - start[0], height: height, r: [5, 5, 5, 5] },
          style: api.style()
        };
      },
      encode: { x: [1, 2], y: 0 },
