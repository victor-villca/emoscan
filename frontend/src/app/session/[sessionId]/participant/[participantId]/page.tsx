...
const emotionTypes = {
  ...
  2: {
    ...
    iconClass: 'ri-emotion-sad-line',
  },
  3: {
    name: 'Angry',
    color: 'rgba(252, 141, 98, 1)',
    iconClass: 'ri-emotion-unhappy-line',
  },
  4: {
    name: 'Surprised',
    color: 'rgba(140, 145, 179, 1)',
    iconClass: 'ri-emotion-line',
  },
  5: {
    name: 'Neutral',
    color: 'rgba(141, 211, 199, 1)',
    iconClass: 'ri-emotion-normal-line',
  },
  6: {
    name: 'Fearful',
    color: 'rgba(187, 157, 206, 1)',
    iconClass: 'ri-emotion-2-line',
  },
  7: {
    name: 'Disgusted',
...
export default function ParticipantReport({
  ...
  useEffect(() => {
    ...
    if (pieChartElement) {
      ...
      const pieData = data?.emotionReport.emotions.map((emotion) => ({
        ...
        name:
          emotionTypes[emotion.emotion_type_id as keyof typeof emotionTypes]
  ...
  const emotionMinutes = emotionReport.emotions.map((emotion) => {
    const minutes = Math.round(
      (parseFloat(emotion.percentage) / 100) * totalSessionMinutes
    );
    return {
      ...emotion,
      minutes,
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              {sessionDate} (9:00 AM - 5:00 PM)
            </p>
          </div>
          <div className="flex gap-4">
            <Link
              href={`/session/${sessionId}`}
              className="flex items-center bg-gray-100 text-gray-700 px-4 py-2 rounded hover:bg-gray-200 transition whitespace-nowrap"
            >
            ...
            <div className="w-full md:w-2/3">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {emotionMinutes.map((emotion) => {
                  const emotionType = emotionTypes[
                    emotion.emotion_type_id as keyof typeof emotionTypes
                  ] || {
                    name: `Type ${emotion.emotion_type_id}`,
                    color: '#ccc',
                    iconClass: 'ri-emotion-line',
                    ...
                    <div key={emotion.id} className="bg-gray-50 rounded p-4">
                      <div className="flex items-center gap-3 mb-2">
                        <div
                          ...
                          style={{
                            backgroundColor: `${emotionType.color}20`,
              ...
              <div className="mt-6 bg-gray-50 rounded p-4">
                ...
                <p className="text-gray-700">
                  During this 8-hour session, {participant.name} primarily
                  displayed
                  {emotionMinutes
                    .sort(
                      (a, b) =>
                    ...
                    {emotionMinutes.sort(
                      ...
                      emotionTypes[
                        emotionMinutes.sort(
                          (a, b) =>
                            parseFloat(b.percentage) - parseFloat(a.percentage)
                        )[0].emotion_type_id as keyof typeof emotionTypes
                      ]?.name}
                  </td>
                </tr>
              </tbody>
        ...
        <div className="bg-white rounded shadow-sm p-6 mb-8">
          ...
          <p className="text-gray-700 mb-4">
            This report provides an analysis of {participant.name}'s emotional
            expressions during the 8-hour session. The data was captured using
            facial expression analysis technology that identifies key facial
            markers to determine the emotional state of the participant.
          </p>
          <div className="bg-blue-50 rounded p-4 text-blue-800 text-sm mt-1">
            ...
            <p>
              Our emotion detection technology measures facial micro-expressions
              to determine emotional states. The system has a 95% accuracy rate
              when compared to human observer assessments. Results should be
              interpreted in context and may be influenced by cultural
