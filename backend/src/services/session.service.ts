...
export const create = async (session: Session) => SessionRepo.createSession(session);
export const getByUser = async (user_id: number) => SessionRepo.getSessionsByUser(user_id);
...
export async function listSessions(userId: number) {
  ...
  export async function getFullSessionReport(sessionId: number) {
    ...
    const participants = await ParticipantRepo.getParticipantsBySession(sessionId);
