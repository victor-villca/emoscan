...
export const createSession = async (session: Omit<Session, 'id'>) => {
  const [created] = await db<Session>('sessions').insert(session).returning('*');
  return created;
};
export const getPaginatedSessionsByUser = async (userId: number, limit: number, offset: number) => {
