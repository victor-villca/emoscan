...
if (typeof window.emoScanInitialized === "undefined") {
  ...
  function initializeUIHandlers() {
    ...
    const liveSessionLinkContainer = document.getElementById(
      "live-session-link-container",
    );
    ...
    window.addEventListener("backendError", (event) => {
      ...
      if (errorBanner && errorBannerText) {
        errorBannerText.textContent =
          event.detail.message || "Connection error.";
          ...
          setTimeout(async () => {
            try {
              ...
              const response = await fetch(
                `http://localhost:4000/api/sessions/${sessionId}/finish`,
                {
                  method: "POST",
                },
              );
              ...
              alert(
                "Error al finalizar la sesión. Por favor, inténtelo desde el dashboard web.",
              );
              statusDot.className = "status-dot active";
              statusText.textContent = "Activa";
              actionText.textContent = "Finalizar Sesión";
  ...
  function setupSessionValidation() {
    ...
    button.addEventListener("click", async () => {
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:4000/api/sessions/validate/${code}`,
          {
            method: "GET",
            headers: {
