...
if (typeof OverlayController === "undefined") {
  class OverlayController {
    ...
    async loadModels() {
      try {
        await faceapi.nets.tinyFaceDetector.loadFromUri(
          chrome.runtime.getURL("js/models"),
        );
      ...
      this.analysisInterval = setInterval(async () => {
        ...
        for (const container of participantContainers) {
          ...
          try {
            ...
            console.warn(
              "Could not extract participant name, falling back to default:",
              e,
            );
          }
        ...
        this.knownParticipants.forEach((data, name) => {
          if (!currentParticipantsInCall.has(name) && data.status !== "left") {
            currentParticipantsInCall.set(name, {
              status: "left",
              videoElement: null,
            });
          }
        ...
        const activeVideoCount = Array.from(
          currentParticipantsInCall.values(),
        ...
        for (const [
          participantName,
          data,
        ] of currentParticipantsInCall.entries()) {
          if (data.status !== "active_with_video") continue;

          const now = Date.now();
          if (
            now - (lastSentTimestamps.get(participantName) || 0) <
            this.SEND_INTERVAL
          ) {
            continue;
          }
          ...
          try {
            ...
            if (detection) {
              if (
                ...
                !participantName.startsWith("Participant_")
              ) {
                lastSentTimestamps.set(participantName, now);
                const faceCanvases = await faceapi.extractFaces(canvas, [
                  detection,
                ]);
                if (faceCanvases.length > 0) {
                  const aiFormattedCanvas = this.convertToAIFormat(
                    faceCanvases[0],
                  );

                  console.log(
                    `Processing participant: "${participantName}" (${videoEl.videoWidth}x${videoEl.videoHeight})`,
                  );
                  await this.sendFaceToBackend(
                    aiFormattedCanvas,
                    window.sessionCode,
                    participantName,
                  );
                ...
                console.log(
                  `Skipping send for default/unidentified participant: "${participantName}"`,
                );
              }
            }
          } catch (error) {
            console.error(
              `Error processing video for participant "${participantName}":`,
              error,
            );
          }
        }
    ...
    async updateAndSendParticipantStatus(currentParticipants) {
      const newStatusPayload = Array.from(currentParticipants.entries()).map(
        ([name, data]) => ({ name, status: data.status }),
      );
      const oldStatusPayload = Array.from(this.knownParticipants.entries()).map(
        ([name, data]) => ({ name, status: data.status }),
      );
      if (
        JSON.stringify(
          newStatusPayload.sort((a, b) => a.name.localeCompare(b.name)),
        ) ===
        JSON.stringify(
          oldStatusPayload.sort((a, b) => a.name.localeCompare(b.name)),
        )
      ) {
        return;
      }
      ...
      try {
        ...
        window.dispatchEvent(
          new CustomEvent("backendError", {
            detail: { message: "Connection lost. Status not updated." },
          }),
        );
      }
    }
    ...
    async sendFaceToBackend(canvas, code, participantName) {
      try {
        ...
        console.log(
          `Sending AI-optimized face data - ${canvas.width}x${canvas.height}, grayscale, ~${sizeKB}KB`,
        );
        ...
        const response = await fetch(
          "http://localhost:4000/api/emotions/ingest",
          {
            method: "POST",
            headers: {
        ...
        window.dispatchEvent(
          new CustomEvent("backendError", {
            detail: { message: "Connection lost. Analysis paused." },
          }),
        );
